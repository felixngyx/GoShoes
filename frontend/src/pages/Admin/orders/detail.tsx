import { ArrowLeft, Printer, RefreshCcw, Truck, ListRestart, CheckCheck } from "lucide-react";
...
const renewPaymentLink = async (orderId: number): Promise<{ success: boolean; data?: { url: string } }> => {
  const token = Cookies.get('access_token');
  const BaseUri = import.meta.env.VITE_API_URL;
    ...
    if (error.response) {
      ...
      switch (error.response.status) {
        ...
        case 404:
          throw new Error('Order not found');
        case 422:
          throw new Error(errorData.message || 'Invalid order status for renewal');
        case 500:
          console.error('Server error details:', errorData);
          throw new Error(errorData.message || 'Server error. Please try again later or contact support');
        default:
          throw new Error(`Failed to renew payment: ${errorData.message || 'Unknown error occurred'}`);
      }
    }
...
const updateOrderStatus = async (orderId: number, status: 'shipping' | 'cancelled' | 'expired' | 'completed'): Promise<OrderResponse> => {
  const token = Cookies.get('access_token');
  const BaseUri = import.meta.env.VITE_API_URL;
    ...
    if (error.response) {
      ...
      switch (error.response.status) {
        ...
        case 404:
          throw new Error('Order not found');
        case 422:
          throw new Error(errorData.message || 'Invalid order status for update');
        case 500:
          throw new Error(errorData.message || 'Server error. Please try again later');
        default:
          throw new Error(`Failed to update status: ${errorData.message || 'Unknown error occurred'}`);
      }
    }
...
const OrderDetails = () => {
  ...
  const handlePrint = () => {
    ...
    const printContent = ReactDOMServer.renderToString(<PrintInvoice order={order} />);
  ...
  const handleRenewLink = () => {
    setConfirmDialog({
      ...
      message: 'Are you sure you want to renew the payment link for this order?',
      onConfirm: async () => {
        if (!id) {
  ...
  const statusMutation = useMutation({
    mutationFn: async ({ orderId, status }: { orderId: number; status: 'shipping' | 'cancelled' | 'expired' | 'completed' }) => {
  ...
  const handleStatusUpdate = async (status: 'shipping' | 'cancelled' | 'expired' | 'completed') => {
    setConfirmDialog({
      open: true,
  ...
  return (
    <div className="container mx-auto p-4 space-y-6 bg-white/80 dark:bg-gray-800/80">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link to="/admin/orders" className="flex items-center gap-1 hover:text-blue-600">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          <Tooltip title={order.status === 'cancelled' || order.status === 'completed' ? "Cannot change status while completed" : ""}>
            <span>
              <Button
          ...
          <Tooltip title={order.status === 'shipping' || order.status === 'completed' ? "Cannot change status while shipping" : ""}>
            <span>
              <Button
          ...
          <Tooltip title={order.status === 'shipping' || order.status === 'completed' || order.status === 'pending' || order.payment.method === 'Ship COD' ? "Cannot renew link while pending and order payment ship COD" : ""}>
            <span>
              <Button
                disabled={order.status === 'shipping' || order.status === 'completed' || order.status === 'pending' || order.payment.method === 'Ship COD'}
                variant="outlined"
                size="small"
          ...
          <Tooltip title={order.status === 'completed' ? "Cannot change status while completed" : ""}>
            <Button
      ...
      <Tooltip title={order.status !== 'shipping' ? "Can only print invoice when order is shipping" : ""}>
        <span>
          <Button
            variant="contained"
            size="small"
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {[
          ...
          <div key={index} className="bg-white p-4 rounded-lg shadow bg-white/80 dark:bg-gray-800/80">
      ...
      <div className="bg-white p-4 rounded-lg shadow bg-white/80 dark:bg-gray-800/80">
                ...
                <tr key={index} className="border-b">
                  ...
                  <td className="py-2">
                    {item.variant ? (
                      ...
                      <span className="text-sm text-gray-400">Non Variable</span>
                    )}
                  </td>
                  <td className="py-2">{item.quantity}</td>
                  <td className="py-2 text-right">
                    {new Intl.NumberFormat('de-DE', { style: 'decimal' }).format(Number(item.price))}đ
                  </td>
                  <td className="py-2 text-right">
                    {new Intl.NumberFormat('de-DE', { style: 'decimal' }).format(Number(item.subtotal))}đ
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="mt-6 space-y-2">
          <div className="flex justify-between font-semibold">
            ...
            <span>{new Intl.NumberFormat('de-DE', { style: 'decimal' }).format(Number(order.total))}đ</span>
          </div>
        </div>
      </div>
