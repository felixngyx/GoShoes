...
import { Search, ChevronDown, MoreHorizontal, Filter, Plus, Trash2, Edit } from 'lucide-react';
...
const DiscountList = () => {
  ...
  const [deleteModal, setDeleteModal] = useState<{isOpen: boolean; discountId: number | null}>({
    isOpen: false,
    discountId: null
  });
                  ...
                  {activeDropdown === discount.id && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-1 w-48 rounded-lg shadow-lg bg-white dark:bg-gray-800 border dark:border-gray-700 py-1 z-30">
                        <button
                          onClick={() => {
                            navigate(`/admin/discounts/update/${discount.id}`);
                        ...
                        <button
                          onClick={() => {
                            setDeleteModal({ isOpen: true, discountId: discount.id });
                            setActiveDropdown(null);
                          }}
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
              ...
              <div className="flex gap-3 justify-end">
                <button
                  onClick={() => setDeleteModal({ isOpen: false, discountId: null })}
                  className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"
                >
                  Cancel
                </button>
                <button
                  onClick={() => deleteModal.discountId && handleDelete(deleteModal.discountId)}
