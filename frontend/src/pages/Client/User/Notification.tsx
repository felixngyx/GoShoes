...
interface Notification {
    ...
    timestamp: string
    isNew?: boolean
    type?: 'feature' | 'collaboration' | 'opportunity' | 'notificationUserTracking';
    order_id?: number
}
...
export default function NotificationsPage() {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                const response = await axiosClient.get('notifications/user-tracking');
    ...
    const handleNotificationClick = (notification: Notification) => {
        ...
        if (notification.type === 'notificationUserTracking') {
            if (notification.order_id) {
                ...
                console.error('order_id is undefined for notification:', notification);
            }
        }
            ...
            <div className="notifications p-4 bg-gray-100 rounded-lg">
                ...
                {notifications.length > 0 ? (
                    notifications.map(notification => (
                        <div key={notification.id} className="notification-item p-2 border-b flex justify-between items-center" onClick={() => handleNotificationClick(notification)}>
                            <div>
                                <p>{notification.message}</p>
                                <span className="text-xs text-gray-500">{notification.timestamp}</span>
                            </div>
                            {notification.isNew ? (
                                <button onClick={() => handleMarkAsRead(notification.id)} className="text-green-500">
                                    <CheckCircle size={20} />
                                </button>
                            ) : null}
                        </div>
