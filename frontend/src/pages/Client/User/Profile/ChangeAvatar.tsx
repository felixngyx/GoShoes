...
const ChangeAvatar: React.FC<ChangeAvatarProps> = ({ profile }) => {
  ...
  const handleUploadToCloudinary = async (file: File) => {
    ...
    try {
      const response = await axios.post('https://api.cloudinary.com/v1_1/doc2vx0k6/image/upload', formData, {
        ...
        <div className="flex flex-col w-full gap-4">
          ...
          <button
            className={`btn btn-xs text-white w-full max-w-[180px] ${
              isUploading ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#40BFFF] hover:bg-[#259CFA]'
            }`}
