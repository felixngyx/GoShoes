...
import { getProductByCateId } from "../../../services/client/product";
...
const ProductListCate = () => {
  ...
  const {
    ...
    queryKey: ["PRODUCT_CATE_ID", id],
    queryFn: async () => await getProductByCateId(Number(id), page, perPage),
  });

  const products = response.data || [];

  const { data: catesDate = [] } = useQuery({
    queryKey: ["BRANDS_KEY"],
    queryFn: async () => await getAllCategories(100, 1),
  });

  const cate = Array.isArray(catesDate?.data) ? catesDate.data : [];
  ...
  useEffect(() => {
    refetch();
  }, [searchTerm, sortType, perPage, page, selectedCate]);
              ...
              <li key={item.id}>
                <Link
                  ...
                  className={`block p-4 text-gray-800 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-all duration-200 ease-in-out ${
                    selectedCate === item.id ? "bg-blue-100 text-blue-600" : ""
                  }`}
                >
                  {item.name}
                </Link>
